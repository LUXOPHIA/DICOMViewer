...
$$L = \begin{cases} 16\ \text{ビット} & \text{短形式：AE, AS, AT, CS, DA, DS, DT, FD, FL, IS, LO, LT, PN, SH, SL, SS, ST, TM, UI, UL, US} \\\\ 32\ \text{ビット（予約 2 バイトの後）} & \text{長形式：OB, OD, OF, OL, OV, OW, SQ, SV, UC, UN, UR, UT, UV} \\\\ 32\ \text{ビット} & \text{暗黙的 VR} \end{cases} \qquad \text{(2)}$$
...
$$w = \mathrm{Word}(v \ll (15 - \mathrm{HighBit})), \qquad s = \begin{cases} \mathrm{SmallInt}(w)  /  2^{16-\mathrm{BitsStored}} & \text{符号あり} \\\\ w \gg (16-\mathrm{BitsStored}) & \text{符号なし} \end{cases} \qquad \text{(3)}$$
...
$$d = \begin{cases} 0 & S = 0 \\\\ 32768 & S = 16 \\\\ b - 2^{S} + 1 & b < 2^{S-1} \quad (\text{MSB} = 0 \Rightarrow \text{負数}) \\\\ b & \text{それ以外} \end{cases} \qquad \text{(4)}$$
