...
//  本ユニットは 大浦拓哉 氏の gamerf パッケージ (dgamma.c / dlgamma.c) の Delphi 移植 (双対数による自動微分対応版)。
...
function RGamma( const X_:TdSingle ) :TdSingle; overload;
function RGamma( const X_:TdDouble ) :TdDouble; overload;
...
function RLnGamma( const X_:TdSingle ) :TdSingle; overload;
function RLnGamma( const X_:TdDouble ) :TdDouble; overload;
...
function RGamma( const X_:TdSingle ) :TdSingle;
var
  K, N :Integer;
  W, Y :TdSingle;
begin
     ...
     begin
          W := X_ - 1;
          for K := 2 to N do W := W * ( X_ - K );
     end
     else
     begin
          W := 1;
          for K := 0 downto N + 1 do Y := Y * ( X_ - K );
     end;

     Result := W / Y;
end;

function RGamma( const X_:TdDouble ) :TdDouble;
var
  K, N :Integer;
  W, Y :TdDouble;
begin
...
function RLnGamma( const X_:TdSingle ) :TdSingle;
const
     A :array[ 0..21 ] of Single = (
...
begin
     ...
     begin
          ...
          Y := ( ( ( ( ( ( ( ( ( ( A[ K      ]   * W + A[ K + 01 ] ) * W + A[ K + 02 ] ) * W
                                 + A[ K + 03 ] ) * W + A[ K + 04 ] ) * W + A[ K + 05 ] ) * W
                                 + A[ K + 06 ] ) * W + A[ K + 07 ] ) * W + A[ K + 08 ] ) * W
                                 + A[ K + 09 ] ) * W + A[ K + 10 ] ) * W;
     ...
     begin
          ...
          Y := ( ( ( ( ( ( ( ( ( ( ( ( B[ K      ]   * T + B[ K + 01 ] ) * T + B[ K + 02 ] ) * T
                                     + B[ K + 03 ] ) * T + B[ K + 04 ] ) * T + B[ K + 05 ] ) * T
                                     + B[ K + 06 ] ) * T + B[ K + 07 ] ) * T + B[ K + 08 ] ) * T
                                     + B[ K + 09 ] ) * T + B[ K + 10 ] ) * T + B[ K + 11 ] ) * T
                                     + B[ K + 12 ] ) * T + B[ K + 13 ];
     end
     else
     if W < 8 then
     begin
          ...
          Y := ( ( ( ( ( ( ( ( ( ( ( C[ K      ]   * T + C[ K + 01 ] ) * T + C[ K + 02 ] ) * T
                                   + C[ K + 03 ] ) * T + C[ K + 04 ] ) * T + C[ K + 05 ] ) * T
                                   + C[ K + 06 ] ) * T + C[ K + 07 ] ) * T + C[ K + 08 ] ) * T
                                   + C[ K + 09 ] ) * T + C[ K + 10 ] ) * T + C[ K + 11 ] ) * T
                                   + C[ K + 12 ];
     end
     else
     begin
          ...
          Y := ( ( ( ( ( D[ 0 ]   * T + D[ 1 ] ) * T + D[ 2 ] ) * T + D[ 3 ] ) * T + D[ 4 ] ) * T
                       + D[ 5 ] ) * V + D[ 6 ];

          Y := Y + ( W - 0.5 ) * Ln( W ) - W;
     end;

     if X_ < 0 then Y := Ln( Pi / Sin( X_ * Pi ) ) - Y;

     Result := Y;
end;

function RLnGamma( const X_:TdDouble ) :TdDouble;
const
     A :array[ 0..21 ] of Double = (
          +9.967270908702825e-5  , -1.9831672170162227e-4 ,
          -0.00117085315349625822, +0.00722012810948319552,
          -0.0096221300936780297 , -0.04219772092994235254,
          +0.16653861065243609743, -0.04200263501129018037,
