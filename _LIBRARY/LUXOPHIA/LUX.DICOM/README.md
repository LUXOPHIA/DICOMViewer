...
L =
\begin{cases}
16\ \text{bit} & \text{short form: AE, AS, AT, CS, DA, DS, DT, FD, FL, IS, LO, LT,}\\
& \qquad\qquad\ \ \text{PN, SH, SL, SS, ST, TM, UI, UL, US} \\
32\ \text{bit, after 2 reserved bytes} & \text{long form: OB, OD, OF, OL, OV, OW, SQ, SV, UC, UN, UR, UT, UV} \\
32\ \text{bit} & \text{implicit VR}
\end{cases}
\tag{2}
```
...
s =
\begin{cases}
\mathrm{SmallInt}(w) \,/\, 2^{16-\mathrm{BitsStored}} & \text{signed} \\
w \gg (16-\mathrm{BitsStored}) & \text{unsigned}
\end{cases}
\tag{3}
```
...
0 & S = 0 \\
32768 & S = 16 \\
b - 2^{S} + 1 & b < 2^{S-1} \quad (\text{MSB} = 0 \Rightarrow \text{negative}) \\
b & \text{otherwise}
\end{cases}
\tag{4}
```
